...
//
// template metaprogramming technique for constructing const int arrays at compile time -- could easily be
// generalized to any constant value type.
//
// MAYBE-TODO: upgrade to the C++17 Way (probably constexpr-if, at the least)
...
public:
  ...
  friend std::ostream& operator<<(std::ostream& os, ck2::fp_decimal<FractionalDigits> fp) {
    os << fp.integral();
